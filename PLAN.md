...
| **P1.9 Runtime Port** | **终裁完成（ADR-015）**：完整版 turingosv4（当前 main，194 门禁）squash 导入 `runtime/`；既有外壳全部保留。R-stage 三审计齐（R1.9 质量 + R1.9B omega triage + 汇总，S3=0、五项编目债务拴定 owning atoms）。**前置序列（本地 Mac，v4 仓侧）**：U 项 PR（banner 回灌等）→ 锚点再验证 protocol → 钉 rev 入 PINS。**D-stage 卡集**：A1_9_1 squash 导入 + workspace 接线；A1_9_2 runtime CI lane（194 门禁 + workspace 测试）+ 边界 grep 谓词 + 基线棘轮入 shipgate；红修复卡待再验证的新红编目后增补。R1.9C（PR #280 narrow triage）并行，不阻塞导入 | **D-stage 开闸**（待锚点再验证钉 rev） |
