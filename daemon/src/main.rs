...
fn main() -> ExitCode {
    ...
    match args.next().as_deref() {
        Some("serve") => {
            let (Some(repo), Some(socket)) = (args.next(), args.next()) else {
                eprintln!("usage: turingosd serve <repo-path> <socket-path>");
        ...
        _ => {
            ...
            eprintln!(
                "turingosd {} - schema {} (commands: snapshot <repo-path> | serve <repo-path> <socket-path>)",
