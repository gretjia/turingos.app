...
final class RadarModelTests: XCTestCase {
    ...
    // MARK: A1_63b — railroad swimlane layout tests

    /// (a) trunk straight spine: isDefault head a2 → a1 → a0 (root); all lane 0,
    ///     rowOf strictly increasing.
    func testRailroadTrunkStraightSpine() throws {
        let branches = [
            BranchFact(projectId: "p", branchRef: "refs/heads/main", headSha: "a2",
                       isDefault: true),
        ]
        let commits = [
            CommitFact(projectId: "p", commitSha: "a0", parentShas: [],
                       branchRef: "refs/heads/main", author: "A",
                       ts: "2026-01-01T08:00:00Z", summary: "root"),
            CommitFact(projectId: "p", commitSha: "a1", parentShas: ["a0"],
                       branchRef: "refs/heads/main", author: "A",
                       ts: "2026-01-01T09:00:00Z", summary: "second"),
            CommitFact(projectId: "p", commitSha: "a2", parentShas: ["a1"],
                       branchRef: "refs/heads/main", author: "A",
                       ts: "2026-01-01T10:00:00Z", summary: "third"),
        ]
        let (rowOf, laneOf, branchLane) = RadarLayout.railroadLanes(
            branches: branches, commits: commits)

        // a0 is the Kahn root; isDefault head=a2 → chain a2→a1→a0.
        XCTAssertEqual(laneOf["a0"], 0, "trunk root must be in lane 0")
        XCTAssertEqual(laneOf["a1"], 0, "trunk second commit must be in lane 0")
        XCTAssertEqual(laneOf["a2"], 0, "trunk head must be in lane 0")

        // rowOf: a0 < a1 < a2 (topological order, ts ascending = same order here).
        let r0 = rowOf["a0"]!, r1 = rowOf["a1"]!, r2 = rowOf["a2"]!
        XCTAssertLessThan(r0, r1, "rowOf must be strictly increasing along the chain")
        XCTAssertLessThan(r1, r2, "rowOf must be strictly increasing along the chain")

        // Default branch header is at lane 0.
        XCTAssertEqual(branchLane["refs/heads/main"], 0)
    }

    /// (b) branch lane distinct: feat/wip head a1 parent a0 (non-default) → lane ≥ 1.
    func testRailroadBranchLaneDistinct() throws {
        let branches = [
            BranchFact(projectId: "p", branchRef: "refs/heads/main", headSha: "a0",
                       isDefault: true),
            BranchFact(projectId: "p", branchRef: "refs/heads/feat/wip", headSha: "a1",
                       isDefault: false),
        ]
        let commits = [
            CommitFact(projectId: "p", commitSha: "a0", parentShas: [],
                       ...
                       ts: "2026-01-01T08:00:00Z", summary: "root"),
            CommitFact(projectId: "p", commitSha: "a1", parentShas: ["a0"],
                       branchRef: "refs/heads/feat/wip", author: "B",
                       ts: "2026-01-01T09:00:00Z", summary: "wip"),
        ]
        let (_, laneOf, branchLane) = RadarLayout.railroadLanes(
            branches: branches, commits: commits)

        XCTAssertEqual(laneOf["a0"], 0, "trunk commit must be lane 0")
        XCTAssertGreaterThanOrEqual(laneOf["a1"] ?? -1, 1,
            "non-trunk commit must be in lane ≥ 1")

        XCTAssertEqual(branchLane["refs/heads/main"], 0, "default branch header at lane 0")
        XCTAssertGreaterThanOrEqual(branchLane["refs/heads/feat/wip"] ?? -1, 1,
            "non-default branch header must be in lane ≥ 1")
        XCTAssertNotEqual(branchLane["refs/heads/main"], branchLane["refs/heads/feat/wip"],
            "branch lanes must be distinct")
    }

    /// (c) TOPO-not-ts: child has ts EARLIER than its parent but parentShas=[parent] →
    ///     rowOf[child] > rowOf[parent] (topological order wins over timestamp).
    func testRailroadTopoNotTs() throws {
        let branches: [BranchFact] = []
        let commits = [
            CommitFact(projectId: "p", commitSha: "parent_sha", parentShas: [],
                       branchRef: "refs/heads/main", author: "A",
                       ts: "2026-01-01T10:00:00Z", summary: "parent"),
            CommitFact(projectId: "p", commitSha: "child_sha", parentShas: ["parent_sha"],
                       branchRef: "refs/heads/main", author: "A",
                       ts: "2026-01-01T09:00:00Z", // EARLIER than parent!
                       summary: "child with bad clock"),
        ]
        let (rowOf, _, _) = RadarLayout.railroadLanes(branches: branches, commits: commits)

        let rParent = rowOf["parent_sha"]!
        let rChild  = rowOf["child_sha"]!
        XCTAssertLessThan(rParent, rChild,
            "topological order must place parent before child even when child.ts < parent.ts")
    }

    /// (d) tie-break determinism: two siblings with equal ts → order by commitSha asc;
    ///     two consecutive derive() calls produce identical canonicalDump.
    func testRailroadTieBreakDeterminism() throws {
        let branches = [
            BranchFact(projectId: "p", branchRef: "refs/heads/main", headSha: "p0",
                       isDefault: true),
        ]
        let commits = [
            CommitFact(projectId: "p", commitSha: "p0", parentShas: [],
                       branchRef: "refs/heads/main", author: "A",
                       ts: "2026-01-01T08:00:00Z", summary: "root"),
            // Both siblings have the same ts → must be ordered by sha asc.
            CommitFact(projectId: "p", commitSha: "sibling_aa", parentShas: ["p0"],
                       branchRef: "refs/heads/main", author: "A",
                       ts: "2026-01-01T09:00:00Z", summary: "aa"),
            CommitFact(projectId: "p", commitSha: "sibling_bb", parentShas: ["p0"],
                       branchRef: "refs/heads/main", author: "B",
                       ts: "2026-01-01T09:00:00Z", summary: "bb"),
        ]
        let (rowOf1, _, _) = RadarLayout.railroadLanes(branches: branches, commits: commits)
        let (rowOf2, _, _) = RadarLayout.railroadLanes(branches: branches, commits: commits)

        // Equal ts → tiebreak by sha asc: "sibling_aa" < "sibling_bb".
        let rAA = rowOf1["sibling_aa"]!
        let rBB = rowOf1["sibling_bb"]!
        XCTAssertLessThan(rAA, rBB, "tie-break must be sha ascending: aa < bb")

        // Second call identical.
        XCTAssertEqual(rowOf1, rowOf2, "railroadLanes must be deterministic")

        // Full scene: two consecutive derives give the same dump.
        var ledger = WorktreeLedger()
        ledger.apply(envelope(0, "ProjectRegistered",
            #"{"project_id":"det_p","local":true,"path":"/repos/det_p"}"#))
        let bEvt = """
        {"event_id":"e1","seq":1,"ts":"2026-01-01T00:00:00Z","schema_version":"tos.app.event.v0",\
        "kind":"BranchObserved","source":"github","trust_state":"observed_unsigned",\
        "payload":{"project_id":"det_p","branch_ref":"refs/heads/main","head_sha":"dp0",\
        "is_default":true,"merged_into_default":false,"provenance":"github_api"}}
        """
        ledger.apply(try JSONDecoder().decode(EventEnvelope.self, from: Data(bEvt.utf8)))
        let d1 = RadarScene.derive(ledger: ledger).canonicalDump()
        let d2 = RadarScene.derive(ledger: ledger).canonicalDump()
        XCTAssertEqual(d1, d2, "two derive() calls on the same ledger must be identical")
    }

    /// (e) partial DAG: commit whose only parent is off-scene → finite position, no crash.
    func testRailroadPartialDagNoCrash() throws {
        let branches = [
            BranchFact(projectId: "p", branchRef: "refs/heads/main", headSha: "sha_a",
                       isDefault: true),
        ]
        let commits = [
            // sha_a's only parent is "sha_missing" which is NOT in the commits list.
            CommitFact(projectId: "p", commitSha: "sha_a", parentShas: ["sha_missing"],
                       branchRef: "refs/heads/main", author: "A",
                       ts: "2026-01-01T09:00:00Z", summary: "orphan window"),
        ]
        let (rowOf, laneOf, _) = RadarLayout.railroadLanes(branches: branches, commits: commits)

        // Must not crash; sha_a gets a finite row and lane.
        XCTAssertNotNil(rowOf["sha_a"], "partial-DAG commit must receive a row assignment")
        XCTAssertNotNil(laneOf["sha_a"], "partial-DAG commit must receive a lane assignment")

        // Full scene must produce a finite position.
        var ledger = WorktreeLedger()
        ledger.apply(envelope(0, "ProjectRegistered",
            #"{"project_id":"partial_p","local":true}"#))
        let bEvt = """
        {"event_id":"e1","seq":1,"ts":"2026-01-01T00:00:00Z","schema_version":"tos.app.event.v0",\
        "kind":"BranchObserved","source":"github","trust_state":"observed_unsigned",\
        "payload":{"project_id":"partial_p","branch_ref":"refs/heads/main","head_sha":"s1",\
        "is_default":true,"merged_into_default":false,"provenance":"github_api"}}
        """
        let cEvt = """
        {"event_id":"e2","seq":2,"ts":"2026-01-01T00:00:00Z","schema_version":"tos.app.event.v0",\
        "kind":"CommitObserved","source":"github","trust_state":"observed_unsigned",\
        "payload":{"project_id":"partial_p","commit_sha":"s1","parent_shas":["s0_off_scene"],\
        "branch_ref":"refs/heads/main","author":"A","ts":"2026-01-01T09:00:00Z","summary":"c"}}
        """
        ledger.apply(try JSONDecoder().decode(EventEnvelope.self, from: Data(bEvt.utf8)))
        ledger.apply(try JSONDecoder().decode(EventEnvelope.self, from: Data(cEvt.utf8)))

        let scene = RadarScene.derive(ledger: ledger)
        let commitNode = scene.nodes.first { $0.id == "commit:partial_p:s1" }
        XCTAssertNotNil(commitNode, "partial-DAG commit must appear as a scene node")
        if let pos = scene.positions[commitNode!.id] {
            XCTAssertTrue(pos.x.isFinite && pos.y.isFinite,
                "partial-DAG commit must have finite position (no NaN/inf)")
        } else {
            XCTFail("partial-DAG commit node must have a position")
        }
    }

    /// (f) forest degrade: 0 isDefault → no lane 0, all branches distinct lanes ≥ 1;
    ///     >1 isDefault → each isDefault branch gets a distinct lane (0, 1, …).
    func testRailroadForestDegrade() throws {
        // 0 isDefault case.
        let branches0: [BranchFact] = [
            BranchFact(projectId: "p", branchRef: "refs/heads/alpha", headSha: "sa",
                       isDefault: false),
            BranchFact(projectId: "p", branchRef: "refs/heads/beta", headSha: "sb",
                       isDefault: false),
        ]
        let commits0 = [
            CommitFact(projectId: "p", commitSha: "sa", parentShas: [],
                       branchRef: "refs/heads/alpha", author: "A",
                       ts: "2026-01-01T08:00:00Z", summary: "a"),
            CommitFact(projectId: "p", commitSha: "sb", parentShas: [],
                       branchRef: "refs/heads/beta", author: "B",
                       ts: "2026-01-01T09:00:00Z", summary: "b"),
        ]
        let (_, laneOf0, branchLane0) = RadarLayout.railroadLanes(
            branches: branches0, commits: commits0)

        // No lane 0 for any branch (no trunk).
        XCTAssertGreaterThanOrEqual(branchLane0["refs/heads/alpha"] ?? -1, 1,
            "0-isDefault: branch alpha must be lane ≥ 1")
        XCTAssertGreaterThanOrEqual(branchLane0["refs/heads/beta"] ?? -1, 1,
            "0-isDefault: branch beta must be lane ≥ 1")
        // Branch lanes must be distinct.
        XCTAssertNotEqual(branchLane0["refs/heads/alpha"], branchLane0["refs/heads/beta"],
            "0-isDefault: branches must be on distinct lanes")
        // No commit should be on lane 0.
        XCTAssertGreaterThanOrEqual(laneOf0["sa"] ?? -1, 1,
            "0-isDefault: commit must not be on lane 0")
        XCTAssertGreaterThanOrEqual(laneOf0["sb"] ?? -1, 1,
            "0-isDefault: commit must not be on lane 0")

        // >1 isDefault case.
        let branches2: [BranchFact] = [
            BranchFact(projectId: "p", branchRef: "refs/heads/main", headSha: "m0",
                       isDefault: true),
            BranchFact(projectId: "p", branchRef: "refs/heads/release", headSha: "r0",
                       isDefault: true),
        ]
        let commits2 = [
            CommitFact(projectId: "p", commitSha: "m0", parentShas: [],
                       branchRef: "refs/heads/main", author: "A",
                       ts: "2026-01-01T08:00:00Z", summary: "main"),
            CommitFact(projectId: "p", commitSha: "r0", parentShas: [],
                       branchRef: "refs/heads/release", author: "B",
                       ts: "2026-01-01T09:00:00Z", summary: "release"),
        ]
        let (_, _, branchLane2) = RadarLayout.railroadLanes(
            branches: branches2, commits: commits2)

        // Each isDefault branch must have a distinct lane.
        let mainLane = branchLane2["refs/heads/main"]
        let releaseLane = branchLane2["refs/heads/release"]
        XCTAssertNotNil(mainLane, ">1-isDefault: main must have a lane")
        XCTAssertNotNil(releaseLane, ">1-isDefault: release must have a lane")
        XCTAssertNotEqual(mainLane, releaseLane,
            ">1-isDefault: isDefault branches must not collapse onto one lane")
    }
