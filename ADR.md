...
## ADR-019 两尺度主权内核（Two-Scale Sovereign Kernel）——内部微观 ChainTape ≠ 外部宏观 GitHub（用户 2026-06-15 终裁，Veto-AI 全票核准，/goal 授权自主执行）
依据 `research/R_v06_directive.md`（用户终裁指令 + Veto-AI 宪法联合审计 `[VETO-AI VERDICT_SIGNATURE: APPROVED_AND_ENFORCED]`）。纠正白皮书 v0.5 把宪法 Art. 0 映射到产品层时的**系统性尺度混淆**：v0.5 把 TuringOS 内部微观 ChainTape 与用户项目 GitHub PR/CI/merge 链路混成一层，导致 `HEAD_t` / `wtool` / `∏p` / `tape_t` 全篇抓向外部项目仓库。本 ADR 立两尺度模型为白皮书 v0.6 主骨。
- **A. 核心定理（不可谈判，白皮书 §0 / §19 item 22）**：**Internal ChainTape is the sovereign micro-ledger. GitHub is the external macro execution substrate. A GitHub commit/PR/merge is not a ChainTape node; it is a macro artifact crystallized from many micro nodes and then anchored back into ChainTape as provenance.** 内部 ChainTape = 反奥利奥内核自己的 canonical 微观账本（一次 tick / 提案 / 谓词判定 = 一个内部节点，失败也入带）；GitHub = 外部宏观执行场与交付晶体。
- **B. 尺度剥离（承宪法 Art. 0.4 路径 B）**：宪法路径 B 的物理底座是「每 cell run 用 runtime 临时/内部 git repo；Node = commit object；bus.append = git commit；HEAD_t = git HEAD ref」——这是**内部**微观 ChainTape 的实现底物，**不是**用户项目仓库。故 `HEAD_t` = internal accepted-world pointer，**永不**等同 GitHub merge SHA；`wtool = internal bus.append()`，**永不**等同 git commit/merge（除非显式内部 runtime repo）；`project_git_head`（用户项目 PR/SHA/merge）只能作 provenance / macro artifact anchor 入带。Veto-AI 实证此读法**正确**，非误读宪法。
- **C. 双游标解 Rubber 死锁（承 Art. 0.1 + 0.2，不改宪法一字）**：`tape_tip`（append log，每 micro tick 无条件前进、失败也推进，落实 Art. 0.2「失败也是状态」）与 `accepted_head`（accepted world head，仅 ∏p_micro=1 时前进，落实 Art. 0.1「失败不上升为新状态」）。append 无条件、accepted-world advance 有条件——两条宪法不变量各自成立、互不冲突。
- **D. 两尺度内核方程（白皮书 §8）**：**Micro Tick**——`node = bus.append(...)` 无条件，`accepted_head advances iff Πp_micro=1`；**Macro Boundary**——`macro_merge_allowed iff Πp_macro=1 AND budget/stop-loss/provenance gates AND (autonomy_contract_allows OR human_signature_valid)`。`∏p=1` 在宏观 merge 层只是**必要不充分**条件（签名漏门修复）。merge SHA 作 macro artifact anchor 锚回 ChainTape provenance，**不是** accepted_head。
- **E. 图间连接契约 + 批准完整性（白皮书 §7.0.1 / §9 / §19 item 23）**：所有 flow chart 只能经类型化 ChainTape 过缝事件传递控制（最少 12 类过缝事件，含 `MacroArtifactProposed`/`MacroObservationImported` 为唯一合法 micro↔macro 缝）；禁止经 UI local state / agent memory / parallel cache / GitHub side effect 直接过缝。**Approval Integrity Law**：签名绑定的 canonical 字节必须逐字节等于下游谓词门消费的对象，绝不允许"签 UI 摘要、校验另一个 JSON"。
- **F. 教义纠偏（白皮书 §5/§11/§13/§14/§18）**：① §11 Reward 把 `predicate_pass`/`user_approval` 从连续奖励算子移出、升格 hard gates（防 Goodhart，Art. III.4）；② Meta AI = InitAI 的 law-compilation embodiment（编译期顶层白盒、执行期中层黑盒，权能随 mode 变）；③ ArchitectAI 落盘分三层（ordinary artifact / protocol-contract change 走签名#7+ratification / constitutional substrate 走 sudo#8）；④ provenance 四级（FULL_ACTION / REPO_LEVEL / PARTIAL / OUTSIDE_GOVERNANCE）；⑤ 协议层铁律补预算/止损门；⑥ 路线图 Minimal Sovereign Kernel 前移 M1，Protocol Gateway 不得先于最小 ChainTape 成为实际执行路径。
- **G. 12 条不变量法典化**：`research/R_v06_directive.md §七` 的 12 条验收测试 = 后续所有 ArchitectAI 提案的自动化拦截军规。本期（Phase A）以机械谓词 `scripts/predicates/audit_whitepaper_v06.sh`（grep 项 1/2/5/6/10/11 + 脊柱锚点）+ 独立 clean-context agent（语义项 3/4/7/8/9/12）双轨递归审计落地；硬编码进顶层白盒 Predicate Pack 属 Phase B。
- **边界（不重开）**：本 ADR 是**白皮书层**裁决（Phase A：只改 WHITEPAPER.md + 归档 + 本 ADR + 审计装置）。**Phase B**（契约包：`tape_node` 加 scale/node_kind 字段、approval_card/signed_decision/init_spec_package/budget_autonomy_contract/project_ready/macro_artifact_anchor schema、provenance 四级 enum、12 类 flow_edge_event registry）与 **Phase C**（minimal sovereign kernel 前移实现）各自另立 atom，走 Veto + compatibility + 签名#7 / ratification，**不在本卡**。不碰 `runtime/` trust-root（ADR-015）；不改宪法（substrate/Q_t 语义变更走 sudo#8）；零跨项目边（ADR-009）。
- **落点**：`WHITEPAPER.md` v0.6（Two-Scale Sovereign Kernel Correction）；`research/R_v06_directive.md`（审计规范，逐字归档终裁 + Veto-AI 报告）；`scripts/predicates/audit_whitepaper_v06.sh`（机械递归审计谓词）；`specs/atoms/A1_14_whitepaper_v06.md`（Phase A atom 卡）。按首行纪律走 RATIFICATION_POLICY 立案，不静默改既有 ADR。
